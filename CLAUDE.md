...
- **Découverte des amis Steam** (`POST /api/friends/resolve-steam-ids`, `{ steamIds: string[] }`) : demandé après coup côté utilisateur ("je veux voir tous mes amis Steam qui ont joué à Wyrdane") — le client envoie les SteamID64 de sa liste d'amis Steam locale (`SteamService.get_steam_friend_ids`, jamais transmise ailleurs), `friendModel.resolveSteamIds` renvoie ceux qui ont un compte Wyrdane (`linked_accounts`), bornés à `MAX_STEAM_IDS` (200) côté modèle ET côté contrôleur (rejet 400 direct si dépassé, pour ne pas silencieusement tronquer un payload suspect). Alimente une section "Amis Steam" dédiée dans `FriendsPanel.gd`, distincte de la liste d'amis Wyrdane déjà acceptés — permet d'ajouter en un clic un ami Steam qui a déjà un compte, sans avoir à le chercher par pseudo.
