...
Déjà en place : auth Steam, gestion des decks (CRUD), catalogue de cartes, collection persistante + monnaie molle + boutique de packs, classement ranked (MMR Elo, double-report de match, leaderboard), file d'attente de matchmaking classé par MMR (voir « Matchmaking classé » ci-dessous), niveau de compte par XP (voir « Niveau de compte (XP) » ci-dessous, a remplacé l'ancien barème d'or par match classé), boutique de cosmétiques (ledger d'achats Steamworks Microtransactions), quêtes quotidiennes (voir « Quêtes quotidiennes » ci-dessous), récompense de connexion quotidienne (voir « Récompense de connexion quotidienne » ci-dessous), quêtes hebdomadaires et parrainage à sens unique (voir « Quêtes hebdomadaires & parrainage » ci-dessous), quêtes uniques (voir « Quêtes uniques » ci-dessous), historique de parties + place au classement du profil (voir « Historique de parties & place au classement » ci-dessous), système d'amis Wyrdane + chat + présence (voir « Amis, chat et présence » ci-dessous).

### Historique de parties & place au classement

`GET /api/profile` renvoie désormais aussi `ranked.totalPlayers` (`profileModel.getTotalRankedPlayers`, `COUNT(*)` sur `ranked_stats` de la saison) à côté de `ranked.rank` déjà existant — sert à afficher « #12 / 348 joueurs » côté client (onglet « Principal » du profil, voir `E:\card-game\CLAUDE.md`).

`GET /api/ranked/matches/history?limit=` (défaut/plafond 20/50, `rankedController.getMatchHistoryHandler`) renvoie les dernières parties RÉSEAU (ranked + partie rapide, aucune distinction backend entre les deux — voir « Matchmaking classé » ci-dessus) du joueur authentifié, adversaire le plus récent en tête (`rankedModel.getMatchHistory`) : `client_match_id`, `played_at`, `duration_sec`, `winner_id`, `mmr_change` (déjà résolu côté SQL selon que l'appelant est player1/player2 sur chaque ligne — jamais le MMR brut de l'adversaire), `opponent_username`, `opponent_deck_races` (JOIN sur `match_reports.reporter_id` = l'adversaire ; `null` si son rapport ne portait pas encore `deckRaces`, jamais bloquant). Solo/IA non couverts (pas de second rapporteur pour confirmer un match, voir `match_history`).

`match_history` gagne trois colonnes additives (`sync.ts`, même pattern que `xp_awarded_player1/2`) : `mmr_change_player1/2` (delta Elo signé de CE match, calculé dans `confirmMatch` avant l'écrasement du MMR) et `duration_sec` (déclaré par le rapporteur via `POST /api/ranked/matches/report` → `durationSec`, borné par `matchPayload.sanitizeDurationSec`, 0 si absent — un ancien client qui ne l'envoie pas encore journalise simplement 0).

**Profil d'un ami** : `GET /api/profile/:userId` (`profileController.getFriendProfile`) renvoie exactement le même format que `GET /api/profile` mais pour un AUTRE joueur — réutilise `profileModel.getProfile(targetId)` tel quel (déjà paramétré par id, aucun changement de modèle nécessaire). Restreint aux amis acceptés (`friendModel.findFriendship`, 403 sinon) sauf si `userId` == l'appelant lui-même (self, toujours autorisé) : contrairement au leaderboard public (`GET /api/ranked/leaderboard`, ouvert à tout joueur connecté), ce profil expose l'historique de parties de la cible, jugé plus sensible que le seul pseudo/MMR public du classement. Même principe pour l'historique de parties : `GET /api/ranked/matches/history/:userId` (`rankedController.getFriendMatchHistoryHandler`), miroir de `GET /api/ranked/matches/history` paramétré par id et gaté par la même vérification d'amitié — consommés ensemble par `ProfilePanel.gd` côté `card-game` pour rendre le profil d'un ami avec le même code que le sien.
...
- **Amis Steam ajoutés automatiquement** (`POST /api/friends/resolve-steam-ids`, `{ steamIds: string[] }`) : demandé côté utilisateur le 2026-09-24 puis précisé le 2026-09-25 ("je ne veux pas qu'on ait à les rajouter en jeu") — le client envoie les SteamID64 de sa liste d'amis Steam locale (`SteamService.get_steam_friend_ids`, jamais transmise ailleurs), `friendModel.autoAddSteamFriends` résout ceux qui ont un compte Wyrdane (`resolveSteamIds`, `linked_accounts`) PUIS crée directement la relation en `status = 'accepted'` pour chacun (`autoAcceptFriendship`, sans étape "pending" — la confiance est déjà établie par Steam, redemander une acceptation serait une friction inutile). Idempotent, rappelé à chaque ouverture du panneau Amis (`FriendsPanel._fetch_steam_friends` côté client). Bornés à `MAX_STEAM_IDS` (200) côté modèle ET côté contrôleur (rejet 400 direct si dépassé, pour ne pas silencieusement tronquer un payload suspect).
- **Invitation de partie entre amis** (`game_invites`, `inviteModel.ts`/`inviteController.ts`/`inviteRouter.ts`, table demandée côté utilisateur le 2026-09-25) : remplace l'ancien flux "Contre un ami" par overlay Steam natif (`SteamTransport.invite_friends`/`activateGameOverlayInviteDialog`), retiré côté client. L'expéditeur héberge d'abord son lobby Steam (comme avant), puis `POST /api/invites` (`{ recipientId, steamLobbyId }`) enregistre l'invitation — refusée (409, `not_friends`/`recipient_unavailable`) si les deux joueurs ne sont pas amis acceptés, ou si le destinataire n'est pas en ligne ou déjà `in_game` (vérifié côté serveur, pas seulement côté client). Une seule invitation pending par expéditeur à la fois (`createInvite` annule la précédente). `GET /api/invites/incoming` est pollé côté destinataire (même cadence que le chat) pour afficher la popup d'acceptation avec choix de deck ; `POST /api/invites/:id/accept` (renvoie `steamLobbyId` à rejoindre) / `decline` / `cancel` (côté expéditeur). `INVITE_EXPIRY_SECONDS` (45s, `inviteModel.ts`) expire paresseusement une invitation sans réponse, pas de job planifié — même principe que `matchmaking_tickets`/`TICKET_EXPIRY_SECONDS`.
- **Côté client `card-game`** : `card-game` PR #625 (`worktree-0614-friends-chat`) — panneau Amis (avec section "Amis Steam"), fenêtre de chat, menu contextuel clic droit, `PresenceService` (heartbeat) ; invitation de partie ajoutée ensuite (voir ci-dessus et `E:\card-game\CLAUDE.md`).
...
Table `matchmaking_tickets` (`matchmakingModel.ts`) : **un seul ticket actif par joueur** (`UNIQUE KEY` sur `user_id`), un nouvel appel à `joinQueue` remplace le précédent plutôt que d'en empiler un second (`INSERT ... ON DUPLICATE KEY UPDATE`, réinitialise `created_at`). Pas de job planifié séparé pour l'appariement : `joinQueue` et `getQueueStatus` (à chaque poll client, toutes les 2s côté `NetLobby.gd`) tentent chacun un appariement immédiat parmi les tickets `waiting`, verrouillés `FOR UPDATE` le temps de la transaction pour qu'un même adversaire ne soit jamais apparié deux fois par des requêtes concurrentes. Fenêtre de tolérance MMR élargie progressivement (±100 au départ, +50 toutes les 15s, plafond ±500) — la fenêtre retenue entre deux tickets est la plus large des deux côtés, pour qu'un joueur qui attend depuis longtemps élargisse effectivement ses chances. **Choix de l'adversaire parmi les candidats éligibles (2026-09-25)** : la fenêtre ne dit que *qui est acceptable*, pas *qui est le meilleur*. `findOpponent` retenait jusque-là le premier candidat dans l'ordre `created_at`, donc toujours le ticket le PLUS ANCIEN et jamais le MMR le plus proche — avec plusieurs joueurs en file, un adversaire à 90 points d'écart passait devant un à 20 points s'il attendait depuis plus longtemps. Les candidats éligibles sont désormais classés par `pairingScore` (le plus bas gagne), qui combine les deux critères demandés :

```
score = |Δmmr| − WAIT_BONUS_MMR_PER_SECOND × attente_du_candidat   (plafonné à WAIT_BONUS_MAX_MMR)
```

Le bonus exprime l'ancienneté **en points de MMR** : à 5/s plafonné à 300, un candidat qui patiente depuis 20s « vaut » 100 points d'imprécision, donc un ticket qui traîne finit toujours par passer devant sans qu'on ait à élargir la fenêtre. Celle-ci reste une contrainte **dure** (un adversaire hors fenêtre n'est jamais rattrapé par son ancienneté), et à égalité de score le ticket le plus ancien gagne. Réglage à valider avec de vrais joueurs : avec 2 ou 3 personnes en file, n'importe quelle pondération donne le même résultat, seule la population rend ce score réellement observable. Un ticket `waiting` sans appariement après 5 min passe `expired` (le client abandonne de son côté après 3 min, `RANKED_QUEUE_TIMEOUT`, donc cette valeur n'est jamais le facteur limitant en pratique).
...
### Quêtes de progression nouveaux joueurs (onboarding, niveau 1-25)

Table `onboarding_quests` (`onboardingQuestModel.ts`, `ONBOARDING_QUEST_TEMPLATES` en code) : même structure que `unique_quests` (une ligne par joueur/`quest_code`, assignée une fois pour toutes, jamais reset), mais réservée aux joueurs en cours de montée de niveau — `GET /api/quests/onboarding` renvoie le catalogue entier sous le niveau 25 (`ONBOARDING_LEVEL_CAP`) et, au-delà, **uniquement les quêtes déjà validées** (`progress >= target`) non encore réclamées : la piste disparaît pour un vétéran, mais une récompense déjà acquise avant de dépasser le cap reste réclamable indéfiniment (décision produit : ne pas pénaliser un joueur qui n'a juste pas ouvert le menu à temps).

11 quêtes : 5 paliers `reach_level` (5/10/15/20/25, 100 à 400 or + jusqu'à 2 packs), 3 paliers `buy_packs` (1/5/10 packs **achetés** avec de l'or — `packModel.openPack` uniquement quand `free === false`, contrairement à `open_packs` côté `unique_quests` qui compte aussi les packs gratuits), `win` (5 victoires, tout mode), `win_ranked` (1 victoire réseau) et `play_network` (1 partie réseau jouée, gagnée ou perdue). ~2050 or + 4 packs au total sur toute la piste — pensé comme un coup de pouce de démarrage, pas comme une nouvelle source de farm long terme.

`ensureOnboardingQuests` **backfille** directement les paliers `reach_level` déjà dépassés au moment de l'insertion (comparaison avec le niveau courant du joueur) : un joueur déjà niveau 12 quand cette piste est déployée reçoit d'emblée les paliers 5 et 10 comme acquis, plutôt que de ne plus jamais pouvoir les valider (le hook de progression normal ne se déclenche que sur un futur passage de niveau réel, pas rétroactivement). Un vétéran bien au-delà du niveau 25 récupère ainsi, à sa première ouverture du menu Quêtes après déploiement, les paliers `reach_level` en attente de réclamation (il les a par définition déjà tous atteints) mais aucun des autres objectifs, jamais insérés avec une progression positive.

Branchée dans les mêmes points d'entrée que les autres pistes (`rewardsController.reportSoloMatch`, `rankedController.reportMatch` pour `win`/`win_ranked`/`play_network`), plus deux points d'entrée propres : `levelModel.applyXp` (après `grantLevelReward`, un appel par niveau franchi — volontairement hors de la transaction XP en cours, même choix que `packModel.progressForPackOpen`) pour `reach_level`, et `packModel.openPack` (uniquement sur un achat réel) pour `buy_packs`. `POST /api/quests/onboarding/:id/claim` (verrouillé `FOR UPDATE`, même garde-fou que `claimQuest`/`claimUniqueQuest`).
...
Effacer le cookie ne suffisait pas : un JWT émis avant la suppression reste valide jusqu'à 1h, donc une seconde session (jeu et site ouverts en parallèle) continuait d'agir sur le compte effacé et pouvait y recréer quêtes, présence, decks. `middleware/deletedAccount.ts` tient une table en mémoire des comptes supprimés depuis moins d'une heure, consultée par `authorization` (`middleware/auth.ts`) — pas de requête en base sur le chemin chaud, l'entrée expirant d'elle-même puisque au-delà d'une heure aucun token d'avant la suppression ne peut plus être valide. Même hypothèse mono-process que le limiteur de débit.
...
- **`app.set("trust proxy", 1)`** (`app.ts`) est indispensable derrière Nginx : sans lui, `req.ip` vaut l'adresse du proxy pour toutes les requêtes et le limiteur de débit par IP met tout le monde dans le même seau — un seul client pouvait alors épuiser le quota de `/api/auth/steam` et empêcher n'importe qui de se connecter. `1` et non `true`, pour que `X-Forwarded-For` ne soit pas usurpable par le client.
  ...
  **Ce réglage suppose que Nginx envoie bien l'en-tête.** À vérifier une fois dans `/etc/nginx/sites-available/api.wyrdane.com` : le bloc `location` doit contenir `proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;` (et, tant qu'on y est, `proxy_set_header X-Real-IP $remote_addr;`). Sans cette ligne côté proxy, `trust proxy` ne change rien et le limiteur reste inopérant, sans aucun signe visible. Contrôle rapide (route publique, pas d'auth) : `for i in $(seq 1 130); do curl -s -o /dev/null -w '%{http_code}
' -X POST -H 'Content-Type: application/json' -H 'X-Forwarded-For: 203.0.113.7' -d '{"path":"/test"}' https://api.wyrdane.com/api/analytics/pageview; done | sort | uniq -c` doit finir par des 429 (quota : 120/10 min), et une IP factice différente doit repartir à zéro — si les deux partagent le même quota, l'en-tête n'arrive pas jusqu'à Express.
