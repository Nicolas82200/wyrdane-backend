...
XP requise pour passer du niveau `n` à `n+1` : `xpToReachNextLevel(n)` croît de 5 % par niveau sur le seuil **arrondi** du niveau précédent (100 XP au niveau 1, 105 au niveau 2, 110 au niveau 3...) — pas un calcul en flottant recalculé depuis la base à chaque appel, pour que l'arrondi de chaque palier reste stable. Récompense à chaque niveau franchi (`rewardKindForLevel`, un seul niveau peut normalement être franchi par match, mais `applyXp` boucle pour en gérer plusieurs si jamais `XP_WIN_NETWORK` grandissait) :
