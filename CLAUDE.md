...
XP requise pour passer du niveau `n` à `n+1` : `xpToReachNextLevel(n) = 100 + 10*(n-1)` (courbe légèrement croissante). Récompense à chaque niveau franchi (`rewardKindForLevel`, un seul niveau peut normalement être franchi par match, mais `applyXp` boucle pour en gérer plusieurs si jamais `XP_WIN_NETWORK` grandissait) :
